...
const Fitness = ({ firstAlgorithm, firstFunction, algorithms, functions, handleDeleteClick, handleTestAlgorithmClick, handleTestFunctionClick, algorithmTestRunning, fitnessTestRunning, progress, results, sessions, resumeSession, setPDFDownloadLink, abortSession }) => {
  const [openDeleteModal, setOpenDeleteModal] = useState(false);
  const [selectedItemId, setSelectedItemId] = useState('');
  const [selectedAlgorithmTest, setSelectedAlgorithmTest] = useState(firstAlgorithm);
  const [selectedFunctionsTest, setSelectedFunctionsTest] = useState([]);
  const [selectedFunctionTest, setSelectedFunctionTest] = useState(firstFunction);
      ...
      <div className='grid grid-cols-2 gap-x-8'>
        <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          {algorithmTestRunning === true
            ? <Fragment>
              <div className="p-6.5">
                <h1 className='font-bold text-3xl text-center'>Testowanie algorytmu trwa...</h1>
                <div className="flex mt-32 items-center justify-center bg-white">
                  <div className="h-16 w-16 animate-spin rounded-full border-4 border-solid border-primary border-t-transparent"></div>
                </div>
                <div className="mt-16">
                  {progress.map((p) => {
                    return <p className='text-sm mb-1'>Testowanie funkcją {functions.find(f => f.id === p.fitnessFunctionId).name}... {p.progress}%</p>
                  })}
                </div>
              </div>
            </Fragment>
              ...
              <div className="p-6.5">
                {
                  results.length > 0 ? results.map((res) => {
                    return <div className='grid grid-cols-2'>
                      <h1 className='font-bold text-3xl text-center'>Testowanie zakończone!</h1>
                      <div className="mt-16">
                        <div className="w-full mb-4">
                          ...
                          <p className="text-lg font-bold">
                            {algorithms.find(a => a.id === res.AlgorithmId).name}
                          </p>
                        ...
                        <div className="w-full mb-4">
                          ...
                          <p className="text-lg font-bold">
                            {functions.find(f => f.id === res.FitnessFunctionId).name}
                          </p>
                        </div>
                        <div className="w-full mb-4">
                          <p className="block text-black dark:text-white">
                    ...
                    : <Fragment>
                      ...
                      <div className="p-6.5">
                        <div className="mb-4.5 flex flex-col gap-6 xl:flex-row">
                          <div className="w-full">
                            ...
                            <select name="algorithm"
                              ...
                              onChange={(ev) => setSelectedAlgorithmTest(ev.target.value)}
                        ...
                        <div className="mb-16">
                          ...
                          {functions.map((fun) => {
                            return <div className='grid grid-cols-2 mb-4'>
                              <Checkbox id={fun.id} name={fun.name} changeHandler={addSelectedFunctionCheckbox} />
                            </div>
                          })}
                        </div>
                        <button className="flex w-full justify-center rounded bg-primary p-3 font-medium text-white" onClick={() => {
                          handleTestAlgorithmClick(selectedAlgorithmTest, selectedFunctionsTest);
                        }}>
                          Testuj algorytm!
                        </button>
                      </div>
        ...
        <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          {fitnessTestRunning === true
            ? <Fragment>
              <div className="p-6.5">
                <h1 className='font-bold text-3xl text-center'>Testowanie funkcji trwa...</h1>
                <div className="flex mt-32 items-center justify-center bg-white">
                ...
                <div className="mt-16">
                  ...
                  {progress.map((p) => {
                    return <p className='text-sm mb-1'>Testowanie algorytmem {algorithms.find(a => a.id === p.algorithmId).name}... {p.progress}%</p>
                  })}
                </div>
              </div>
            </Fragment>
            : <Fragment>
              ...
              <div className="p-6.5">
                ...
                <div className="mb-16">
                  ...
                  {algorithms.map((alg) => {
                    return <div key={alg.id} className='grid grid-cols-2 mb-4'>
                      <Checkbox id={alg.id} name={alg.name} changeHandler={addSelectedAlgorithmCheckbox} />
                    </div>
                  })}
                </div>
                <button className="flex w-full justify-center rounded bg-primary p-3 font-medium text-white" onClick={() => {
                  handleTestFunctionClick(selectedFunctionTest, selectedAlgorithmsTest);
                }}>
                  Testuj funkcję!
                </button>
              </div>
                    ...
                    <div className="flex items-center space-x-3.5">
                      ...
                      {session.state === "FINISHED" &&
                        <button className="hover:text-primary" onClick={() => {
                          ...
                          <svg xmlns="http://www.w3.org/2000/svg" height="16" width="16" viewBox="0 0 512 512"><path opacity="1" fill="#1E3050" d="M288 32c0-17.7-14.3-32-32-32s-32 14.3-32 32V274.7l-73.4-73.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3l128 128c12.5 12.5 32.8 12.5 45.3 0l128-128c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L288 274.7V32zM64 352c-35.3 0-64 28.7-64 64v32c0 35.3 28.7 64 64 64H448c35.3 0 64-28.7 64-64V416c0-35.3-28.7-64-64-64H346.5l-45.3 45.3c-25 25-65.5 25-90.5 0L165.5 352H64zm368 56a24 24 0 1 1 0 48 24 24 0 1 1 0-48z" /></svg>
                        </button>
                      }
                          ...
                          <div className="text-center">
                            ...
                            <div className="flex justify-center gap-4">
                              <Button color="failure" onClick={() => {
                                ...
                                sessions.splice(
                                  sessions.findIndex(a => a.sessionId === selectedItemId),
                                  1
                                )
                                setSelectedItemId('');
